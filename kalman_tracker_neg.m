...
for i=1:length(video)
    ...
    idx = find([s.Area] < median*5); %% removing object with more than median·5 pixles (Debris) 
    s1 = s(idx,:);
    mean = mean([s1.Area]);
    end
    idx = find([s.Area] < median*8); %% removing object with more than median·8 pixles (Debris) 
    s1 = s(idx,:); 
    idx1 = find([s1.Area]< mean*2.5 & [s1.Area]> mean*0.4);
    s2 = s1(idx1,:); 
...
noise_mag = 1.5; %process noise: the variability in how fast the cells are speeding up
...
Ex = [dt^4/4 0 dt^3/2 0; ...
    0 dt^4/4 0 dt^3/2; ...
    dt^3/2 0 dt^2 0; ...
    0 dt^3/2 0 dt^2].*noise_mag^2; % Ex convert the process noise (stdv) into covariance matrix
...
C = [1 0 0 0; 0 1 0 0];  %measurement function C, that we apply to the state estimate Q to get our expect next/new measurement
...
for i=1:length(loc_cellX)
    ...
    for j=2:length(loc_cellX{1,i})-1
    ...
    VCL(i,1) = sum(d(i,:));
    VAP(i,1) = sum(d_mean(i,:)+norm(([loc_cellX{1,i}(1,end-1);loc_cellY{1,i}(1,end-1)])-([loc_cellX{1,i}(1,end);loc_cellY{1,i}(1,end)]))+norm(([loc_cellX{1,i}(1,1);loc_cellY{1,i}(1,1)])-([loc_cellX{1,i}(1,2);loc_cellY{1,i}(1,2)])));
